...
export default function UserPreferenceList() {
  ...
  useEffect(() => {
    ...
    const result = preferences.filter(
      (p) =>
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `user-preferences-${format(
      new Date(),
      "yyyy-MM-dd_HH-mm"
    )}.csv`;
    a.click();
      ...
      <div>
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">
            Create orders directly for users who submitted preferences
          </p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-[#faf5e9] border-b-2 border-gray-300">
                <tr>
                  ...
                  <th className="px-12 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                    Genres
                  </th>
                  <th className="px-7 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                    Book Request
                  </th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                    Duration
              ...
              <tbody className="divide-y divide-gray-200">
                {filtered.length === 0 ? (
                  <tr>
                    <td
                      colSpan={8}
                      className="text-center py-20 text-gray-500 text-lg font-medium"
                    >
                      {search
                        ? "No preferences match your search."
                        : "No user preferences submitted yet."}
                    </td>
                  </tr>
                ) : (
                  filtered.map((pref) => (
                    <tr
                      key={pref.id}
                      className="hover:bg-[#fefcf7] transition-all duration-200 group"
                    >
                      ...
                      <td className="px-2 py-5">
                        <div className="flex flex-wrap gap-2">
                          {pref.genres.map((g) => (
                            <span
                              key={g}
                              className="px-3 py-1.5 text-xs font-bold text-black bg-[#EEFF00] rounded-full shadow-sm"
                            >
                              {g}
                            </span>
                          ))}
                      ...
                      <td className="px-1 py-5">
                        <div className="text-[13px] font-medium">
                          {format(pref.preferredDates.when, "MMM d")} –{" "}
                          {format(pref.preferredDates.till, "MMM d, yyyy")}
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <button
                          onClick={() => openOrderModal(pref)}
                          className="px-2 py-2   
                          rounded-xl  
                          whitespace-nowrap
                          bg-[#F7F3EA] text-black 
                          text-sm 
          ...
          <div className="lg:hidden">
            {filtered.length === 0 ? (
              ...
              filtered.map((pref) => (
                <div
                  key={pref.id}
                  className="border-b border-gray-200 p-6 hover:bg-[#fefcf7] transition-all"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button
                      onClick={() => openOrderModal(pref)}
                      className="px-5 py-3 bg-black text-white font-bold rounded-xl hover:bg-gray-900 transition-all shadow-md flex items-center gap-2 text-sm"
                    >
                  ...
                  <div className="space-y-4 text-sm">
                    <div className="flex flex-wrap gap-2">
                      {pref.genres.map((g) => (
                        <span
                          key={g}
                          className="px-3 py-1.5 text-xs font-bold text-black bg-[#EEFF00] rounded-full"
                        >
                          {g}
                        </span>
                      ))}
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {format(pref.preferredDates.when, "MMM d")} –{" "}
                          {format(pref.preferredDates.till, "MMM d")}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="text-xs text-gray-500 pt-3 border-t border-gray-200">
                      Created:{" "}
                      {format(pref.createdAt, "MMM d, yyyy 'at' h:mm a")}
                    </div>
                  </div>
                </div>
              ))
        ...
        <div className="mt-6 text-center text-sm text-gray-600">
          ...
          <strong>{preferences.length}</strong> preference
      ...
      {isModalOpen && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[95vh] overflow-hidden">
            ...
            <div className="px-6 overflow-y-auto max-h-[85vh]">
              <CreateOrderForm
                initialUser={{
